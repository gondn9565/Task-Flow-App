...
import { CheckCircle, MoreVertical, Calendar, Clock } from "lucide-react";
...
const TaskItem = ({
  ...
  const handleSave = async (updatedTask) => {
    try {
      ...
      if (err.response?.status === 404) onLogout?.();
    }
  };
            ...
            <div className={TI_CLASSES.menuDropdown}>
              {MENU_OPTIONS.map((opt) => (
                <button
                  ...
                  className="w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm hover:bg-purple-50 flex items-center gap-2 transition-color duration-200"
                >
                  {opt.icon}
                  {opt.label}
                </button>
